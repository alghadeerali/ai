...
import { useGetConversation, useListMessages, useSendMessage, useUpdateConversation, useCreateConversation, useArchiveConversation, useListModels, getGetConversationQueryKey, getListMessagesQueryKey, getListConversationsQueryKey } from "@workspace/api-client-react";
import { useQueryClient } from "@tanstack/react-query";
import { Send, Edit2, Archive, UserSquare, Bot, Mic, Sparkles, Brain, ChevronDown, Paperclip, X, Plus, Menu, Trash2 } from "lucide-react";
...
export default function ChatPage() {
  ...
  useEffect(() => { if (conversation && !isEditingTitle) { setEditedTitle(conversation.title); setSelectedModel(conversation.model); } }, [conversation, isEditingTitle]);
  useEffect(() => { if (scrollRef.current) scrollRef.current.scrollTop = scrollRef.current.scrollHeight; }, [messages?.length]);
  const handleSaveTitle = () => { if (!id || !editedTitle.trim()) return; updateConv.mutate({ id, data: { title: editedTitle.trim() } }, { onSuccess: () => setIsEditingTitle(false) }); };
  ...
  return <div className="flex min-h-[100dvh] w-full overflow-hidden bg-background text-foreground">
    ...
    <main className="flex min-w-0 flex-1 flex-col overflow-hidden">
      <div className="md:hidden flex h-14 items-center justify-between border-b border-border bg-background px-3 sticky top-0 z-20">
      ...
      <div className="border-t border-border bg-background px-3 py-3 pb-[max(0.75rem,env(safe-area-inset-bottom))]">
        <div className="mx-auto flex w-full max-w-3xl flex-col gap-2">
          <div className="flex items-center gap-2 overflow-x-auto pb-1 no-scrollbar">
            ...
            <Button type="button" variant="outline" size="icon" className="h-9 w-9 shrink-0 rounded-full" onClick={handleAttachClick} aria-label="المرفقات"><Paperclip className="h-4 w-4" /></Button>
            <Button type="button" variant="outline" size="icon" className="h-9 w-9 shrink-0 rounded-full" onClick={toggleRecording} aria-label="إدخال صوتي"><Mic className="h-4 w-4" /></Button>
            <Select value={selectedModel} onValueChange={setSelectedModel}><SelectTrigger className="h-9 w-[150px] shrink-0 rounded-full bg-muted/40 px-3 text-xs"><SelectValue placeholder="النموذج" /></SelectTrigger><SelectContent>{visibleModels.map((m: any) => <SelectItem key={m.id} value={m.id}>{m.name}</SelectItem>)}</SelectContent></Select>
            <Button type="button" variant="outline" size="sm" className="h-9 shrink-0 rounded-full px-3 text-xs" onClick={() => setThinking((v) => !v)}><Brain className="mr-1 h-4 w-4" />تفكير</Button>
          </div>
          <div className="rounded-[1.5rem] border border-input bg-card shadow-[0_10px_30px_rgba(0,0,0,0.06)]">
            ...
            <div className="flex items-center justify-between gap-2 px-3 pb-3"><div className="flex items-center gap-2"><Button type="button" variant="ghost" size="icon" className="h-10 w-10 rounded-full" onClick={handleAttachClick} aria-label="المرفقات"><Paperclip className="h-4 w-4" /></Button><Button type="button" variant="ghost" size="icon" className="h-10 w-10 rounded-full" onClick={toggleRecording} aria-label="التسجيل"><Mic className="h-4 w-4" /></Button><Button type="button" variant="ghost" size="icon" className="h-10 w-10 rounded-full md:hidden" onClick={() => setShowActions((v) => !v)} aria-label="المزيد"><Plus className="h-4 w-4" /></Button></div><Button type="button" onClick={handleSend} disabled={sendMessage.isPending || !input.trim()} className="h-11 w-11 rounded-full p-0"><Send className="h-4 w-4" /></Button></div>
          </div>
          {showActions && <div className="rounded-2xl border border-border bg-card p-3 text-sm md:hidden"><DropdownMenu open onOpenChange={setShowActions}><DropdownMenuTrigger asChild><button className="hidden" /></DropdownMenuTrigger><DropdownMenuContent><DropdownMenuItem onClick={() => { setShowActions(false); toast.message('سلسلة التفكير متاحة من زر تفكير'); }}><Brain className="mr-2 h-4 w-4" />التفكير العميق</DropdownMenuItem><DropdownMenuSeparator /><DropdownMenuItem onClick={() => { handleAttachClick(); setShowActions(false); }}><Paperclip className="mr-2 h-4 w-4" />مرفقات</DropdownMenuItem><DropdownMenuItem onClick={() => { handleDeleteConversation(); setShowActions(false); }} className="text-destructive"><Trash2 className="mr-2 h-4 w-4" />حذف المحادثة</DropdownMenuItem></DropdownMenuContent></DropdownMenu></div>}
