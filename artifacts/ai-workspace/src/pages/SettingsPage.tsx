...
export default function SettingsPage({ onClose }: { onClose?: () => void }) {
  ...
  return <div className="mx-auto w-full max-w-5xl p-4 sm:p-6">
    <div className="flex items-center justify-between gap-4">
      <div><h1 className="text-2xl font-bold">الإعدادات</h1><p className="mt-2 text-muted-foreground">كل شيء محفوظ ويظهر في الجوال والديسكتوب.</p></div>
    ...
    <div className="mt-6 grid gap-6 lg:grid-cols-2">
      ...
      <section className="rounded-3xl border border-border bg-card p-4 sm:p-6">
        ...
        <div className="mt-4 space-y-2">
          {imageModels.map((m) => <div key={m.id} className="flex items-center justify-between rounded-2xl border border-border bg-muted/30 px-4 py-3"><div><div className="font-medium">{m.name}</div><div className="text-xs text-muted-foreground">الكلفة التقديرية: {m.cost}</div></div><Button size="sm" variant={m.id === defaultModel ? 'default' : 'outline'} onClick={() => setDefaultModel(m.id)}>اختيار</Button></div>)}
        </div>
      </section>
    </div>
  </div>
